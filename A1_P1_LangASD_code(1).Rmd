...
finalData$Gender=revalue(finalData$Gender,c("1"="F", "2"="M"))

#Renaming diagnosis in the same way
finalData$Diagnosis=as.factor(finalData$Diagnosis)

finalData$Diagnosis=revalue(finalData$Diagnosis,c("A"="ASD", "B"="TD"))
...
#more than 2.7
child_MLU=filter(finalData,CHI_MLU>2.7)
length(child_MLU)
child_MLU$ID
unique(child_MLU$ID)
...
tokensAllVisits = finalData %>% group_by(ID) %>%summarise(meanWordsTotal = mean(tokens_CHI))

mergedData = merge(finalData, tokensAllVisits, by = "ID")
